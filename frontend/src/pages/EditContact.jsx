...
export default function EditContact() {
  ...
  useEffect(() => {
    const fetchContact = async () => {
      try {
        const response = await axios.get(`${API_URL}/contacts/${contactId}`, { headers })
        setFormData({
          name: response.data.name || '',
          firstname: response.data.firstname || '',
          email: response.data.email || '',
          biserica: response.data.biserica || '',
          recomandat_de: response.data.recomandat_de || '',
          tel1: response.data.tel1 || '',
          tel2: response.data.tel2 || '',
          tel3: response.data.tel3 || '',
  ...
  const hasPhoneNumber = () => Boolean(
    normalizeValue(formData?.tel1) || 
    normalizeValue(formData?.tel2) || 
    normalizeValue(formData?.tel3)
  )
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Send PATCH request
      await axios.patch(`${API_URL}/contacts/${contactId}`, data, { headers })
      navigate('/profile')
    } catch (err) {
      ...
      if (typeof detail === 'string') {
        ...
        setError('Failed to update contact. Check your network or backend logs.')
      }
    } finally {
      setLoading(false)
    }
  ...
  if (pageLoading) {
    return <div className="crm-panel p-6 text-center">Loading contact...</div>
  }

  if (!formData) {
    return <div className="crm-panel p-6 text-center text-red-600">Contact not found</div>
  }

  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="crm-hero">
        <p className="crm-label">Edit</p>
        <h1 className="crm-page-title">Edit Contact</h1>
        <p className="crm-page-subtitle">Update the contact information. At least one phone number is required.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="crm-panel space-y-6 p-6 md:p-8">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className="space-y-2">
            ...
            <input 
              type="text" 
              name="biserica" 
              placeholder="Ex: Biserica Sf. Dumitru" 
              value={formData.biserica} 
              onChange={handleChange} 
              className="crm-input" 
            />
          </label>
        ...
        <div className="flex flex-col gap-3 pt-2 md:flex-row">
          ...
          <button type="button" onClick={() => navigate('/profile')} className="crm-button-secondary flex-1">
            Cancel
          </button>
        </div>
      </form>
